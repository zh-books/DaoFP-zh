...
\begin{document}
    ...
    \chapter{Natural Transformations \\ 自然变换}

    我们已经看到，当两个对象 $a$ 和 $b$ 同构（isomorphic）时，它们会在箭头集之间生成双射（bijections），现在我们可以将其表示为同态集（hom-sets）之间的同构（isomorphisms）。对于所有 $x$，我们有：
    \begin{align*}
        \mathcal{C}(a, x) &\cong \mathcal{C}(b, x) \\
        \mathcal{C}(x, a) &\cong \mathcal{C}(x, b)
    \end{align*}
    但逆命题不成立。同态集之间的同构并不会导致对象之间的同构，\emph{除非}附加的自然性条件（naturality conditions）得到满足。我们现在将在更普遍的情况下重新表述这些自然性条件。

    \section{Natural Transformations Between Hom-Functors \\ 同态函子之间的自然变换}

    对象之间的同构可以通过直接提供两个箭头——一个是另一个的逆来建立。但通常间接地通过定义箭头之间的双射要更容易实现，无论是作用于这两个对象的箭头，还是从这两个对象发出的箭头。

    例如，正如我们之前所见，对于每个 $x$，我们可以有一个箭头的可逆映射 $\alpha_x$。
    \[
        \begin{tikzcd}
            \node(x) at (0, 2) {x};
            \node(a) at (-2, 0) {a};
        ...
        \end{tikzcd}
    \]
    换句话说，对于每个 $x$，存在一个同态集之间的映射：
    \[ \alpha_x \colon \mathcal{C}(x, a) \to \mathcal{C}(x, b) \]

    当我们改变 $x$ 时，这两个同态集变成了两个（逆变）函子（contravariant functors），$\mathcal{C}(-, a)$ 和 $\mathcal{C}(-, b)$，而 $\alpha$ 可以被视为它们之间的映射。这样的函子映射，称为变换（transformation），实际上是一组单独的映射 $\alpha_x$，每个对象 $x$ 在范畴 $\mathcal{C}$ 中都有一个。

    函子 $\mathcal{C}(-, a)$ 描述了世界如何看待 $a$，而函子 $\mathcal{C}(-, b)$ 描述了世界如何看待 $b$。

    变换 $\alpha$ 在这两种视角之间切换。$\alpha$ 的每个分量，即 $\alpha_x$ 的双射，表明从 $x$ 的视角看 $a$ 与从 $x$ 的视角看 $b$ 是同构的。

    我们之前讨论的自然性条件是以下条件：
    \[ \alpha_y \circ (- \circ g) = (- \circ g) \circ \alpha_x \]
    它关联了在不同对象上取的 $\alpha$ 的分量。换句话说，它关联了通过箭头 $g \colon y \to x$ 连接的两个观察者 $x$ 和 $y$ 的视角。

    该等式的两边都作用于同态集 $\mathcal{C}(x, a)$。结果在同态集 $\mathcal{C}(y, b)$ 中。我们可以将等式的两边重写为：
    \begin{align*}
        \mathcal{C}(x, a) \xrightarrow{(- \circ g)} \mathcal{C}(y, a) \xrightarrow{\alpha_y} \mathcal{C}(y, b) \\
        \mathcal{C}(x, a) \xrightarrow{\alpha_x}  \mathcal{C}(x, b)  \xrightarrow{(- \circ g)}\mathcal{C}(y, b)
    \end{align*}


    预合成（precomposition）$g \colon y \to x$ 也是同态集的映射。事实上，它是通过逆变同态函子来提升 $g$。我们可以将其分别写作 $\mathcal{C}(g, a)$ 和 $\mathcal{C}(g, b)$。
    \begin{align*}
        \mathcal{C}(x, a) \xrightarrow{\mathcal{C}(g, a)} \mathcal{C}(y, a) \xrightarrow{\alpha_y} \mathcal{C}(y, b) \\
        \mathcal{C}(x, a) \xrightarrow{\alpha_x}  \mathcal{C}(x, b)  \xrightarrow{\mathcal{C}(g, b)}\mathcal{C}(y, b)
    \end{align*}
    ...
    我们现在可以重新表述我们之前的结果：一个在函子 $\mathcal{C}(-, a)$ 和 $\mathcal{C}(-, b)$ 之间满足自然性条件的可逆变换 $\alpha$ 等价于对象 $a$ 和 $b$ 之间的同构。

    我们可以按照完全相同的推理对外发箭头进行处理。这次我们从一个变换 $\beta$ 开始，其分量为：
    \[ \beta_x \colon \mathcal{C}(a, x) \to \mathcal{C}(b, x) \]
    两个（协变）函子 $\mathcal{C}(a, -)$ 和 $\mathcal{C}(b, -)$ 描述了从 $a$ 和 $b$ 的视角来看世界的方式。可逆变换 $\beta$ 告诉我们这两种视角是等价的，而自然性条件
    \[ (g \circ -) \circ \beta_x = \beta_y \circ (g \circ -) \]
    告诉我们当我们改变焦点时，它们的行为是一致的。
    ...
    \section{Natural Transformation Between Functors \\ 函子之间的自然变换}
    ...
    \begin{align*}
        ...
        G x &=   \mathcal{C}(b, x)
    \end{align*}
    它们都将范畴 $\mathcal{C}$ 映射到 $\mathbf{Set}$，因为同态集就在这里。我们可以说它们在 $\mathbf{Set}$ 中创造了 $\mathcal{C}$ 的两种不同的\emph{模型}。
    ...
    \[
        \begin{tikzcd}
            ...
            \\
            x
            \arrow[rru, dashed, "{\mathcal{C}(a, -)}"]
            \arrow[rrd, dashed, "{\mathcal{C}(b, -)}"']
            \\
            && \mathcal{C}(b, x)
        \end{tikzcd}
    \]

    这个想法自然地扩展到任意对范畴之间的函子。任何两个函子
    \begin{align*}
        F &\colon \mathcal{C} \to \mathcal{D} \\
        G &\colon \mathcal{C} \to \mathcal{D}
    \end{align*}
    可以看作是 $\mathcal{C}$ 在 $\mathcal{D}$ 中的两个不同的模型。

    为了将一个模型转化为另一个模型，我们使用 $\mathcal{D}$ 中的箭头连接相应的点。对于 $\mathcal{C}$ 中的每个对象 $x$，我们选择一个从 $F x$ 到 $G x$ 的箭头：
    ...
    模型的结构不仅与对象有关，还与箭头有关，因此让我们看看箭头的情况。对于 $\mathcal{C}$ 中的每一个箭头 $f \colon x \to y$，我们有两个在 $\mathcal{D}$ 中相应的箭头：
    \begin{align*}
        F f &\colon F x \to F y \\
        G f &\colon G x \to G y
    \end{align*}
    这些是 $f$ 的两个提升（liftings），它们让我们在每个模型内部移动。然后还有 $\alpha$ 的分量，它们让我们\emph{在}模型之间切换。

    自然性（naturality）意味着，先在第一个模型内移动再跳到第二个模型，或者先跳到第二个模型再在其中移动，这二者之间的顺序无关紧要。这可以通过交换的\emph{自然性方块}来说明：
    ...
    这种满足自然性条件的 $\alpha_x$ 箭头家族称为\emph{自然变换}。

    这是一个显示了范畴对、它们之间的两个函子以及函子之间的自然变换 $\alpha$ 的交换图：
    \[
    ...
    由于在 $\mathcal{C}$ 中的每一个箭头都有一个相应的自然性方块，我们可以说自然变换将对象映射到箭头，将箭头映射到交换方块。

    如果自然变换的每个分量 $\alpha_x$ 都是同构，那么 $\alpha$ 称为\emph{自然同构}。

    我们现在可以重新表述关于同构的主要结果：当且仅当存在一个它们的同态函子之间的自然同构（无论是协变还是逆变，二者之一即可）时，两个对象是同构的。

    自然变换提供了一种非常方便的高层次方式来表达各种情况下的交换条件。我们将利用它们来重新定义代数数据类型。

    \section{Natural Transformations in Programming \\ 编程中的自然变换}

    自然变换是一组由对象参数化的箭头。在编程中，这对应于一组由类型参数化的函数，即\emph{多态函数}。

    自然变换的参数类型由一个函子描述，而返回类型由另一个函子描述。
