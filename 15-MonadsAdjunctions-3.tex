...
% cap
    ...
    在我们的例子中，我们可以将 \hask{Maybe} 视为内在的monad：
    \[ T a = 1 + a \]
    ...
    \begin{haskell}
        s -> Maybe (a, s)
    \end{haskell}
    这与我们定义的 \hask{MaybeState} monad 一致。
...
% mu
    ...
    让我们逐步解析这些方程，以应用于状态monad变换器。状态monad由柯里化伴随（currying adjunction）生成。左函子 $L_s$ 是乘积函子 \hask{(a, s)}，而右函子 $R_s$ 是指数函子（exponential），又称为读者函子（reader functor）\hask{(s -> a)}。
    ...
    我们将保持内在monad $(T, \eta^i, \mu^i)$ 的任意性。在Haskell中，我们将这个三元组称为 \hask{m}, \hask{return}, 和 \hask{join}。
    ...
    这个公式中有很多细节，让我们逐步分析。从右边开始：我们有伴随的单位元 $a$ 分量，这是从 $a$ 到 $R (L a)$ 的箭头。在Haskell中，这是函数 \hask{unit}。
    \begin{haskell}
        unit :: a -> s -> (a, s)
    \end{haskell}
    让我们在某个 \hask{x :: a} 上计算这个函数。结果是另一个函数 \hask{s -> (a, s)}。我们将这个函数作为参数传递给 $R(\eta^i_{L a})$。

    $\eta^i_{L a}$ 是内在monad的 \hask{return} 在 $L a$ 上的分量。这里，$L a$ 是类型 \hask{(a, s)}。所以我们将多态函数 \hask{return :: a -> m a} 实例化为函数 \hask{(a, s) -> m (a, s)}。（类型推断器会为我们自动执行这个操作。）

    接着，我们用 $R$ 提升这个 \hask{return} 的分量。这里，$R$ 是指数 $(-)^s$，所以它通过后组合（post-composition）来提升函数。它将 \hask{return} 后组合到任何传递给它的函数中。在我们的例子中，这是由 \hask{unit} 生成的函数。注意，类型匹配：我们正在将 \hask{(a, s) -> m (a, s)} 后组合到 \hask{s -> (a, s)} 之后。
    ...
    \end{haskell}
    我们插入了数据构造函数 \hask{StateT} 以使类型检查器满意。这是复合monad的 \hask{return}，是以内在monad的 \hask{return} 表示的。

    同样的推理可以应用于某个 $a$ 处复合 $\mu$ 的分量公式：

    \[ \mu_a = R(\mu^i_{L a}) \circ (R \circ T) (\varepsilon^o_{(T\circ L) a})\]
    内在 $\mu^i$ 是monad \hask{m} 的 \hask{join}。应用 $R$ 将其转化为后组合。

    外在 $\varepsilon^o$ 是取自 $T(L a)$ 或 \hask{m (a, s)} 的函数应用。它是一个类型的函数：
    \begin{haskell}
        (s -> m (a, s), s) -> m (a, s)
    \end{haskell}
    插入适当的数据构造函数后，可以将其写为 \hask{uncurry runStateT}：
    \begin{haskell}
        uncurry runStateT :: (StateT s m a, s) -> m (a, s)
    \end{haskell}
    应用 $(R \circ T)$ 使用函子 $R$ 和 $T$ 的组合来提升 $\varepsilon$ 的分量。前者作为后组合实现，后者是monad \hask{m} 的 \hask{fmap}。

    将这些组合在一起，我们得到了状态monad变换器的 \hask{join} 的无点公式（point-free formula）：
    \begin{haskell}
        join :: StateT s m (StateT s m a) -> StateT s m a
        join mma = StateT (join . fmap (uncurry runStateT) . runStateT mma)
    \end{haskell}
    这里，部分应用的 \hask{(runStateT mma)} 从参数 \hask{mma} 中剥离了数据构造函数：
    \begin{haskell}
        runStateT mma :: s -> m (a, x)
    \end{haskell}

    我们之前的 \hask{MaybeState} 示例现在可以使用monad变换器重写为：
    \begin{haskell}
        type MaybeState s a = StateT s Maybe a
    \end{haskell}

    可以通过将 \hask{StateT} monad变换器应用于身份函子（identity functor）来恢复普通的 \hask{State} monad，身份函子在库中定义了一个 \hask{Monad} 实例（注意，在这个定义中最后的类型变量 \hask{a} 被省略了）：
    \begin{haskell}
        type State s = StateT s Identity
    \end{haskell}

    其他monad变换器遵循相同的模式。它们在monad变换器库（Monad Transformer Library，\hask{MTL}）中定义。
    ...
    考虑前面表达式monad \hask{Ex} 的示例。这个monad的一个代数是选择载体类型，比如 \hask{Char}，以及一个箭头：
    \begin{haskell}
        alg :: Ex Char -> Char
    \end{haskell}
    由于 \hask{Ex} 是一个monad，它定义了一个单位元，或者说 \hask{return}，它是一个多态函数，可以用来从值生成简单的表达式。 \hask{Ex} 的单位元是：
    \begin{haskell}
        return x = Var x
    \end{haskell}
    我们可以为任意类型实例化这个单位元，特别是为我们代数的载体类型。我们合理地要求，\emph{评估} \hask{Var c}（其中 \hask{c} 是一个字符）应该返回相同的 \hask{c}。换句话说，我们希望：
    \begin{haskell}
        alg . return = id
    \end{haskell}
    这个条件将立即消除很多代数，例如：
    ...
    我们想要施加的第二个条件是，与monad兼容的代数应该尊重替换。monad允许我们使用 \hask{join} 扁平化嵌套表达式。代数允许我们评估这些表达式。

    有两种方式可以做到这一点：我们可以将代数应用于一个扁平化的表达式，或者我们可以先应用它到内部表达式（使用 \hask{fmap}），然后再评估结果表达式。
    \begin{haskell}
    ...
    一个monad $(T, \eta, \mu)$ 的Kleisli范畴记作 $\mathcal{C}_T$。它的对象与 $\mathcal{C}$ 的对象相同，但 $\mathcal{C}_T$ 中从 $a$ 到 $b$ 的一个箭头由 $\mathcal{C}$ 中的一个箭头表示，这个箭头从 $a$ 到 $T b$。你可能认出它是我们之前定义的Kleisli箭头 \hask{ a -> m b}。由于 $T$ 是一个monad，这些Kleisli箭头可以使用“鱼”（fish）操作符 \hask{<=<} 进行组合。
